...
resource "null_resource" "control_node_comman" {
  provisioner "local-exec" {
    command = <<-EOT
    ip1= $(sh ../../utils/get_device.sh dc-workshop1 ${data.vault_kv_secret_v2.tailscale_api_key.data["key1"]})
    echo "Script output: $ip1"
...
/*
resource "null_resource" "control_node_command3" {
  provisioner "local-exec" {
    command = <<-EOT
     DEVICE_INFO=$(cat ../../devi.json | jq '.devices[0].addresses[]')
     echo  $DEVICE_INFO > dev.info
    EOT
  }
}
